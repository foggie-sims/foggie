...
def make_df_from_frb(frb, df, projection, args):
    '''
    Function to convert a given FRB array to pandas dataframe, AND plots the FRB as a projection image
...
def plot_Zprof_snap(df, ax, args):
    ...
    for index,thisproj in enumerate(args.projections):
        ...
        linefit, linecov = np.polyfit(x_bin_centers, y_binned, 1, cov=True, w=w_binned) # linear fitting done in logspace
        ...
        try:
            ...
            if not os.path.exists(df_snap_filename) or args.clobber:
                ...
                for index, thisproj in enumerate(args.projections):
                    ...
                    axes_proj_snap[index] = plot_projectedZ_snap(np.log10(weighted_map_Z), thisproj, axes_proj_snap[index], args, clim=args.Zlim, cmap='viridis', color=args.col_arr[index])
