...
def make_table(flux_types, surface_type, args, edge=False):
...
def calc_fluxes(ds, snap, zsnap, dt, refine_width_kpc, tablename, save_suffix, surface_args, flux_types, Menc_profile, args, sat=False, sat_radius=0.,inverse=False, disk=False, Rvir=100., halo_center_kpc2=[0,0,0]):
    '''This function calculates the fluxes specified by 'flux_types' into and out of the surfaces specified by 'surface_args'. It
    uses the dataset stored in 'ds', which is from time snapshot 'snap', has redshift
    'zsnap', and has width of the refine box in kpc 'refine_width_kpc', the time step between outputs
    ...
    *outside* the surfaces given in 'surface_args'. If 'disk' is True, then at least once surface shape
    ...
    edges = False
    if (surface_args[0][0]=='cylinder'):
        table = make_table(fluxes, ['cylinder', surface_args[0][7]], args)
        table_edge = make_table(fluxes, ['cylinder', surface_args[0][7]], args, edge=True)
        edges = True
        if (sat):
            table_sat = make_table(fluxes, ['cylinder', surface_args[0][7]], args, edge=True)
        bottom_edge = surface_args[0][1]
        top_edge = surface_args[0][2]
        cyl_radius = surface_args[0][6]
        ...
        outer_radius = surface_args[0][2]
        num_steps = surface_args[0][3]
        table = make_table(fluxes, ['sphere', 0], args)
        if (surface_args[0][0]=='frustum') or (surface_args[0][0]=='ellipse'):
            table_edge = make_table(fluxes, ['sphere', 0], args, edge=True)
            edges = True
        if (sat):
            table_sat = make_table(fluxes, ['sphere', 0], args, edge=True)
        if (args.units_kpc):
