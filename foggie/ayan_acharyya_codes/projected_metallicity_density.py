...
"""
    ...
    Started :    Aug 2023
    Examples :   run projected_metallicity_density.py --system ayan_local --halo 8508 --upto_kpc 10 --use_density_cut --output RD0030 --docomoving
"""
...
# ----------------------------------------------------------------
def plot_projected(thisfield, box, box_center, box_width, projection, ax, args):
    '''
    ...
    field = field_dict[thisfield]
    proj = yt.ProjectionPlot(box.ds, projection, field, center=box_center, data_source=box, width=box_width * kpc, weight_field=weight_field_dict[field], fontsize=args.fontsize)

    # -----------making the colorbar labels etc--------------
    if islog_dict[field]: proj.set_log(field, True)
    proj.set_unit(field, unit_dict[field])
    proj.set_zlim(field, zmin=bounds_dict[field][0], zmax=bounds_dict[field][1])
    ...
    proj.plots[field].axes = ax
    proj._setup_plots()
    proj.plots[field].axes.set_position(position) # in order to resize the axis back to where it should be
    ...
    cbar = fig.colorbar(proj.plots[field].cb.mappable, orientation='vertical', cax=cax)
    cbar.ax.tick_params(labelsize=args.fontsize)
    cbar.set_label(proj.plots[field].cax.get_ylabel(), fontsize=args.fontsize)

    return proj, ax

# ----------dicts of colors, limits, etc. --------------------------
field_dict = {'gas':('gas', 'density'), 'metal':('gas', 'metallicity')}
unit_dict = {'density': 'g/cm**3', 'metal': r'Zsun'}
islog_dict = defaultdict(lambda: False, metal=True, density=True)
weight_field_dict = defaultdict(lambda: None, metal=('gas', 'mass'))
bounds_dict = defaultdict(lambda: (None, None), density=(1e-31, 1e-21), metal=(1e-3, 1e1))
...
if __name__ == '__main__':
    ...
    for index in range(core_start + args.start_index, core_end + 1):
        ...
        try:
            ...
            fig, axes = plt.subplots(nrow, ncol, figsize=(3 + ncol*2, 2 + nrow*4))
            fig.tight_layout()
            ...
            for i,field in enumerate(fields):
                for j,projection in enumerate(projections):
                    proj, axes[i][j] = plot_projected(field, box, ds.halo_center_kpc, box_width, projection, axes[i][j], args)
            ...
            outfile_rootname = '%s_%s_projected_met_den_%s%s%s.png' % (args.output, args.halo, args.Zgrad_den, upto_text, args.weightby_text)
