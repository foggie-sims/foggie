...
def lookup_grid(paramlist, args):
    ...
    print('min, max h2r distance in kpc', np.min(paramlist['radial_dist']), np.max(paramlist['radial_dist']), '\n') #
   ...
   # -----------------reading grid files onto RGI-------------------------------------------------------------------
    ...
    for diag in args.diag_arr:
        ...
        for Om in args.Om_arr:
            ...
            if args.plot_fluxgrid: ax_fluxgrid = makeplot_fluxgrid(paramlist, photgrid, args, quant1=quant1, quant2=quant2, quan3=quant3, quant4=quant4, plot_suffix='_' + diag + ',Om=' + str(Om) + '_' + args.xratio.replace('/', '|') + '_' + args.yratio.replace('/', '|') + '_grid')

            if args.plot_metgrad: ax_metgrad = makeplot_metgrad(paramlist, Zoutcol, args, plot_suffix='_' + diag + ',Om=' + str(Om) + '_metallicity_gradient')

            if args.plot_Zin_Zout: fig_Zinout = makeplot_Zin_Zout(paramlist, Zoutcol, args, plot_suffix='_' + diag + ',Om=' + str(Om) + '_Zin_vs_Zout')

            if args.plot_phase_space: fig_phase_space = makeplot_phase_space(paramlist, args, plot_suffix='_' + diag + ',Om=' + str(Om) + '_P_vs_r_colorby_logQ')
