...
"""
    ...
    Example :    run projection_plot.py --system ayan_hd --halo 4123 --output RD0038 --do gas --proj x --fullbox --nrot 0 --do_central
...
def do_plot(ds, field, axs, annotate_positions, small_box, center, box_width, cmap, name, unit='Msun/pc**2', \
    ...
    if field[1] == 'age': # then do ParticlePlot
        prj = yt.ParticleProjectionPlot(ds, axs, field, center=center, data_source=small_box, width=box_width_kpc, weight_field=weight_field)
...
def make_projection_plots(ds, center, refine_box, box_width, fig_dir, name, \
    ...
    zmin_dict = defaultdict(lambda: density_proj_min, metal=2.5e-1, temp=1.e3, vrad=-250, ys_age=0.1, ys_mass=1, ys_density=1e-3, gas_entropy=1.6e25)
...
if __name__ == '__main__':
    ...
    for this_sim in list_of_sims:
        ...
        center = refine_box.center.in_units(kpc) if args.do_central else args.halo_center
        ...
        if not args.noplot:
            ...
            for nrot in range(min_nrot, max_nrot):
                ...
                prj = make_projection_plots(ds=refine_box.ds, center=center, \
                                        refine_box=refine_box, box_width=2 * args.galrad, \
