...
#
# Modified version by Molly Peeples for shifting files to lou
#
#-----------------------------------------------------------------------------
...
$email_address = 'molly@stsci.edu'; # put email in singe quotes
$job_file = "RunScript.sh";
$parameter_file = (glob("*.enzo"))[0];
$enzo_executable = "./enzo";
$walltime = 432000;
...
while (1) {
    ...
    #--- automatically shiftc new output to relevant directory on lou; UPDATE PATH HERE
    $directory = (split "/", $last_output)[1];
    $command_line = "shiftc --create-tar $directory lou:halo_002392/nref11c_nref9f/$directory.tar";
    system($command_line);
