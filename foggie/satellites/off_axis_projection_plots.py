...
def do_plot(ds, field, normal, north_vector, annotate_positions, \
    ...
    print (weight_field)
    prj = yt.OffAxisProjectionPlot(ds, normal, field, north_vector = north_vector, center = center, data_source = small_box, width=x_width, weight_field = weight_field)
...
def make_off_axis_projection_plots(ds, center, refine_box, x_width,fig_dir, haloname, normal_vector,north_vector,\
    ...
    if not is_central:
        ...
        small_box = ds.r[center[0] - x_width/np.sqrt(2): center[0] + x_width/np.sqrt(2),
                     center[1]     - x_width/np.sqrt(2): center[1] + x_width/np.sqrt(2),
                     center[2]     - x_width/np.sqrt(2): center[2] + x_width/np.sqrt(2),
                    ]
...
if __name__ == '__main__':
    ...
    #for args.halo, args.output in inputs[:2]:
    #for args.halo, args.output in inputs[2:4]:
    for args.halo, args.output in inputs[4:]:
        ...
        for sat in sat_cat_halo:
            ...
            if True:
                ...
                prj = make_off_axis_projection_plots(ds, sat_center, refine_box,  box_width, fig_dir + '/' + args.halo, haloname,norm_vector, north_vector = sat_velocity_norm.value, \
                                    fig_end = '%s'%(sat['id']), \
            ...
            tot_nrot = 200
            for nrot in np.arange(tot_nrot):
                norm_vector = [0, np.cos(2*pi * (1.*nrot)/tot_nrot), np.sin(2*pi * (1.*nrot)/tot_nrot)]
                prj = make_off_axis_projection_plots(ds, sat_center, refine_box,  box_width, fig_dir + '/' + args.halo, haloname,norm_vector, north_vector = [1,0,0], \
                                    fig_end = '%s_%.3i'%(sat['id'], nrot), \
                                    do = ['gas', 'stars', 'dm', 'temp', 'metal'], axes = ['off_axis'],  annotate_center = True,
                                    add_velocity = False, add_arrow = True, start_arrow = [start_arrow, start_arrow], end_arrow = [end_arrow, end_arrow_2])  
