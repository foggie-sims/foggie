...
def make_table(ion_list, max_pair_deg=5, nlos=5000):
...
def make_plot(ion_list, max_pair_deg=5):
    #### read data tables and plot ! ###
    ...
    for ii, ion_tag in enumerate(ion_list):
        ### read in some table ###
        tb_name = '%s/%s_%s_quastar_logNoffset_maxpairdeg%d_%s.fits'%(fig_dir, sim_name, dd_name, max_pair_deg, ion_tag)
        tb = Table.read(tb_name, format='fits')
        offset_logN = tb['offset_logN']
...
if __name__ == '__main__':
    import sys
    import numpy as np
    max_pair_deg = np.float(sys.argv[1]) # define as the largest separation allowed for a close pair of qso-star sightlines
    nlos = 5000 # 1000
    ion_list = ['HI', 'SiII', 'CII', 'SiIII', 'SiIV', 'CIV', 'NV',
                'OVI', 'NeVII', 'NeVIII', 'OVII', 'OVIII']

    # make_table(ion_list, max_pair_deg=max_pair_deg, nlos=nlos)

    make_plot(ion_list, max_pair_deg=max_pair_deg)
