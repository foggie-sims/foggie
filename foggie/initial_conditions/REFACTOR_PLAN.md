...
**Status: demonstrated, not adopted.** Written up so the option is recoverable if
the performance argument for the 256 box becomes compelling. Do not build on this
without reading the risks at the end.

### The idea

`256`-at-L+1 and `512`-at-L are the same simulation inside the zoom: effective
grid `256·2^(L+1) = 512·2^L`, and finest particle mass `2.100e5` vs `2.098e5`
Msun. The 256 root grid costs 8× fewer cells and particles. That makes the 256
box an attractive cheaper substrate for the same science.

The obstacle is halo *selection*, not resolution. A 256 root grid resolves a
1e9 Msun/h halo with ~13 particles, which is below what Rockstar will report, so
the small halos this project cares about have no 256 catalog entry to zoom on.
Measured for halo80181, at its z = 0 position in the 256 parent run:
...
But the *seeds* that form it are present. Both parent boxes share `seed[5]`
through `seed[13]` identically and differ only in `levelmin`/`levelmax` (8 vs 9),
so MUSIC's level-9 and level-10 white noise -- the modes that build this halo --
are the same field in both. Refine the right volume and the halo appears.

### Why the pipeline cannot do it as written

`enzo-mrp-music` defines the Lagrangian region by finding the parent-box
particles inside a sphere at z = 0 and tracing them back to z = 99. With zero
particles there is nothing to trace, and widening the sphere until it catches
some (400 kpc/h for 121 particles) defines a region roughly twenty times too
large and mostly not the halo.

### What works instead: transplant the region

The region is not a set of particles, it is a **volume in the initial
conditions**, and MUSIC expresses it as a point file in normalized [0,1] box
coordinates:
...
Those coordinates are resolution-independent, so the file transplants between
boxes unchanged. Trace the region once in the 512 box, then hand the same file
to a 256 MUSIC config with `levelmin = 8`.

### The test that was run

`$FOGGIE_ICS_DIR/halo80181-256test/`, halo80181 -- a halo with no 256 catalog
entry and zero 256 particles inside Rvir. Its 512 L1 point file (127 points) was
copied unchanged into a 256 config differing from the 512 one only in
`levelmin`/`levelmax` and the output name.

MUSIC accepted it and derived an identical region:

    region center, 256 test   (0.893808, 0.689565, 0.010886)
    region center, 512 build  (0.893808, 0.689565, 0.010886)

    domain shift, 256 test    (-100,  -48, 125)
    domain shift, 512 build   (-201,  -96, 250)      -- a factor of two, as expected

Bounding boxes differ only by `padding = 4` cells scaling with cell size, and
`region_point_levelmin = 9` against `levelmin = 8` was accepted rather than
...
1. **The tidal field differs.** Inside the refined volume the modes match, but
   the surrounding structure is sampled 8× more coarsely. The halo's assembly
   history is not guaranteed to match its 512 twin, and for a dwarf whose
   merger history is its defining property that is not a small caveat. Expect
   differences at least of order the 100-200 kpc inter-level drift already seen.
2. **The result cannot be validated against a catalog.** There is no 256
   Rockstar entry for these halos, so "did we get the right object" has to be
   answered by comparison against the 512 run rather than independently.
3. **Selection bias enters through the back door.** Targets would be chosen from
   the 512 catalog, so the sample is defined at one resolution and run at
   another. Any statement about completeness or about the halo mass function
   would need care.
4. **It decouples two things the pipeline currently keeps together.** Every
   stage presently derives from its own parent box. This introduces a donor
   relationship between boxes that the registry has no way to express, and that
   `validate-registry` cannot check.

### What adopting it would require

- A registry field naming the donor box and halo, since a row would no longer be
  self-describing.
...
- The donor's 512 build as a prerequisite, which the dependency logic does not
  currently model.
- A validation step comparing the resulting halo against its donor -- mass,
  position, and the projected-density ladder -- because nothing else would catch
  a region transplanted to the wrong place.
