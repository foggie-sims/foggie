...
def emission_velocity_with_pixel_size(ds, refine_box, ions, ions_dict, prefix,
    ...
    target_positions = [
        (yt.YTQuantity(4, "kpc"), yt.YTQuantity(25, "kpc")),
        (yt.YTQuantity(28.1, "kpc"), yt.YTQuantity(23.6, "kpc")),
        (yt.YTQuantity(-39.1, "kpc"), yt.YTQuantity(26.1, "kpc")),
        (yt.YTQuantity(5, "kpc"), yt.YTQuantity(15, "kpc")),
        (yt.YTQuantity(10, "kpc"), yt.YTQuantity(10, "kpc")),
        (yt.YTQuantity(0, "kpc"), yt.YTQuantity(3, "kpc"))]
        ...
        for bin_width_kms in bin_width_kms_list:
            ...
            for ion in ions:
                ...
                for (y_center, z_center) in target_positions:
                    ...
                    velocity_hist_data[ion][key] = (bin_centers, hist)
...
def plot_velocity_comparison_all_pairs(hist_data_for_binwidth, ions,
                                       bin_width_kms=25, normalize=True,
                                       smoothing=False, sigma=1.5):
    """
    Compare all ion pairs using velocity histograms for a specific bin width.

    Parameters:
        hist_data_for_binwidth (dict): velocity_hist_data_all_bins[bin_width_kms]
        ions (list): List of ions to compare
        bin_width_kms (float): Bin width to label plots
        normalize (bool): Normalize to unit area
        smoothing (bool): Apply Gaussian smoothing
        sigma (float): Width for smoothing
    """
    from itertools import combinations
    from scipy.ndimage import gaussian_filter1d

    save_path = os.path.join(prefix, 'velocity_plots/comparison/')
    os.makedirs(save_path, exist_ok=True)

    colors = ['dodgerblue', 'lime', 'tomato', 'purple', 'orange']

    ion_pairs = list(combinations(ions, 2))

    for ion1, ion2 in ion_pairs:
        if ion1 not in hist_data_for_binwidth or ion2 not in hist_data_for_binwidth:
            print(f"Skipping {ion1} vs {ion2} due to missing data.")
            continue

        shared_keys = set(hist_data_for_binwidth[ion1].keys()) & set(hist_data_for_binwidth[ion2].keys())

        for i, (y_kpc, z_kpc) in enumerate(shared_keys):
            bin_centers1, hist1 = hist_data_for_binwidth[ion1][(y_kpc, z_kpc)]
            bin_centers2, hist2 = hist_data_for_binwidth[ion2][(y_kpc, z_kpc)]

            if not np.allclose(bin_centers1, bin_centers2):
                print(f"Bin mismatch between {ion1} and {ion2} at ({y_kpc},{z_kpc})")
                continue

            if normalize:
                norm1 = np.sum(hist1 * bin_width_kms)
                norm2 = np.sum(hist2 * bin_width_kms)
                hist1 = hist1 / norm1 if norm1 > 0 else hist1
                hist2 = hist2 / norm2 if norm2 > 0 else hist2

            if smoothing:
                hist1 = gaussian_filter1d(hist1, sigma=sigma)
                hist2 = gaussian_filter1d(hist2, sigma=sigma)

            fig, ax = plt.subplots(figsize=(10, 4.5))
            ax.plot(bin_centers1, hist1, label=ion1, color=colors[0])
            ax.plot(bin_centers2, hist2, label=ion2, color=colors[1])
            ax.set_title(f"{ion1} vs {ion2} @ y={y_kpc:.1f}, z={z_kpc:.1f}", fontsize=16)
            ax.set_xlabel("LOS Velocity (km/s)", fontsize=16)
            ax.set_ylabel("Normalized Emissivity" if normalize else "Emissivity", fontsize=16)
            ax.set_xlim(-600, 400)
            ...
            filename = f'velocity_comparison_{ion1}_vs_{ion2}_y{y_kpc:.1f}_z{z_kpc:.1f}_bin{bin_width_kms}.png'
            plt.tight_layout()
            plt.savefig(os.path.join(save_path, filename), dpi=300)
            plt.close(fig)

    print(f"Finished velocity comparisons for bin width {bin_width_kms} km/s.")

def plot_3x3_velocity_comparison_grid(velocity_hist_data_all_bins, ions,target_positions,
                                      bin_widths=[200, 100, 30],
                                      normalize=True, smoothing=False, sigma=1.5):
    """
    Compare one reference ion (ions[0]) with others (ions[1:]) across multiple bin widths.
    Generates a grid of plots (rows = ion comparisons, columns = bin widths).

    Parameters:
        velocity_hist_data_all_bins (dict): bin_width -> {ion -> {(y,z): (bin_centers, hist)}}
        ions (list): List of ions (first one is the reference)
        bin_widths (list): List of bin widths (e.g., [200, 100, 30])
        normalize (bool): Normalize histograms to unit area
        smoothing (bool): Apply Gaussian smoothing
        sigma (float): Width for smoothing
    """
    ...
    ref_ion = ions[0]
    compare_ions = ions[1:]
    ion_pairs = [(ref_ion, ion) for ion in compare_ions]
    ion_color_dict = {
    'HI': '#0C5DA5',
    'OVI': '#FF9F00',
    'CIII': '#00B945',
    ...
    # First, find (y,z) pairs that exist for all ions across all bin widths
    all_keys = None
    for bin_width in bin_widths:
        hist_data = velocity_hist_data_all_bins[bin_width]
        keys_per_ion = [set(hist_data[ion].keys()) for ion in ions if ion in hist_data]
        if len(keys_per_ion) < len(ions):
            continue
        shared_keys = set.intersection(*keys_per_ion)
        if all_keys is None:
            all_keys = shared_keys
        else:
            all_keys &= shared_keys

    if not all_keys:
        print("No shared (y,z) locations across all ions and bin widths.")
        return


    fig, axes = plt.subplots(len(compare_ions), len(bin_widths), figsize=(15, 10), sharex=True, sharey=True)
    plt.subplots_adjust(wspace=0.01, hspace=0.0)
    axes = np.array(axes)

    for row, (ion2, (y_kpc, z_kpc)) in enumerate(zip(compare_ions, target_positions)):
        ion1 = ref_ion
        for col, bin_width in enumerate(bin_widths):
            ax = axes[row, col] if axes.ndim > 1 else axes[max(row, col)]
            hist_data = velocity_hist_data_all_bins[bin_width]

            try:
                key = (float(y_kpc), float(z_kpc))
                bin_centers1, hist1 = hist_data[ion1][key]
                bin_centers2, hist2 = hist_data[ion2][key]

            except KeyError:
                continue

            if not np.allclose(bin_centers1, bin_centers2):
                print(f"Bin mismatch between {ion1} and {ion2} at bin {bin_width} and ({y_kpc}, {z_kpc})")
                continue

            if normalize:
                hist1 = hist1 / np.sum(hist1 * bin_width) if np.sum(hist1) > 0 else hist1
                hist2 = hist2 / np.sum(hist2 * bin_width) if np.sum(hist2) > 0 else hist2

            if smoothing:
                hist1 = gaussian_filter1d(hist1, sigma=sigma)
                hist2 = gaussian_filter1d(hist2, sigma=sigma)

            #ax.plot(bin_centers1, hist1, label=ion1, color=ion_color_dict.get(ion1, 'black'), linestyle='-', lw=2)
            #ax.plot(bin_centers2, hist2, label=ion2, color=ion_color_dict.get(ion2, 'gray'), linestyle='-', lw=2)
            ax.step(bin_centers1, hist1, label=ion1, color=ion_color_dict.get(ion1, 'black'), linestyle='-', lw=2)
            ax.step(bin_centers2, hist2, label=ion2, color=ion_color_dict.get(ion2, 'gray'), linestyle='-', lw=2)

            for spine in ax.spines.values():
                spine.set_linewidth(1.5)

            if row == 0:
                ax.set_title(f"$\Delta$v: {bin_width} km/s", fontsize=23)
            if row == 2:
                ax.set_xlabel("LOS Velocity (km/s)", fontsize=23)
            if col == 0:
                ax.set_ylabel("Emissivity", fontsize=23)
    ...
    # Save
    print(f"Saving 3x3 comparison grid plots to {save_path}")
    fname = f"velocity_comparison_grid_per_ion_target.png"
    plt.tight_layout(rect=[0, 0, 0.95, 0.95])
    plt.savefig(os.path.join(save_path, fname), dpi=300, bbox_inches='tight')
    plt.close(fig)

def plot_emissivity_weighted_velocity_maps_with_sightlines(ds, refine_box, snap, ions, target_positions,
                                                            unit_system='photons', filter_type=None,
                                                            filter_value=None, res_arcsec=None):
    """
    Plots and saves emissivity-weighted LOS velocity projection maps (edge-on and face-on)
    with annotated sightlines.

    Parameters:
        ds (yt.Dataset): The dataset object
        refine_box (yt.data_object): The region to analyze
        snap (str): Snapshot name
        ions (list): List of ions to process
        target_positions (list): List of (y_kpc, z_kpc) tuples to annotate
        unit_system (str): 'photons' or 'erg'
        filter_type (str): Filtering criteria, e.g., 'disk_cgm'
        filter_value (float or None): Filter cutoff value
        res_arcsec (float or None): Angular resolution in arcsec
    """
    print("Starting emissivity-weighted velocity projection plots (with sightlines)")
    ...
    for region, data_source in data_sources.items():
        for ion in ions:
            print(f"Generating projection maps for {ion} in {region}")

            emission_field = ('gas', f'Emission_{ions_dict[ion]}')
            cmap = cmr.get_sub_cmap('cmr.viola', 0.2, 0.8)
            cmap.set_bad(cmap(0))

            # Add velocity fields if not already present
            ds.add_field(('gas', 'vx_disk_cgs'),
             function=lambda field, data: data[('gas', 'vx_disk')].to('cm/s'),
             units='cm/s', sampling_type='cell', force_override=True)

            ds.add_field(('gas', 'vz_disk_cgs'),
                        function=lambda field, data: data[('gas', 'vz_disk')].to('cm/s'),
                        units='cm/s', sampling_type='cell', force_override=True)
            ...
            for (y_kpc, z_kpc) in target_positions:
                color = f"C{i % 10}"  # Use matplotlib's built-in cycle
                ax.plot(y_kpc, z_kpc, marker='s', markersize=8, markerfacecolor='none',
                        markeredgecolor=color, markeredgewidth=2)


            plt.tight_layout()
            region_save_path = os.path.join(save_path, region)
            plt.savefig(region_save_path + f'{snap}_{ion}_vlos_map_edge-on_{region}' + save_suffix + '.png', dpi=300)
            plt.close()
            ...
            for (x_kpc, y_kpc) in target_positions:
                color = f"C{i % 10}"
                ax.plot(x_kpc, y_kpc, marker='s', markersize=8, markerfacecolor='none',
                        markeredgecolor=color, markeredgewidth=2)


            plt.tight_layout()
            plt.savefig(region_save_path + f'{snap}_{ion}_vlos_map_face-on_{region}' + save_suffix + '.png', dpi=300)
            plt.close()

    print("Finished emissivity-weighted velocity projection plots.")
...
def make_delta_v_maps_between_ions(ds, refine_box, snap, ions, region, data_source,target_positions,
                                     emission_field_dict,prefix,filter_value=None, cmap_range=(0, 300),
                                     save_suffix=''):
    """
    Compute and save delta-v maps (absolute difference in emissivity-weighted LOS velocity)
    between each pair of ions for a given projection.

    Parameters:
        ds: yt dataset
        refine_box: yt data object
        snap: snapshot label
        ions: list of ions to compare
        region: string label (e.g. 'cgm')
        data_source: yt data container for the region
        width_value: float (in kpc), field of view size
        res: int, resolution of the projection map
        emission_field_dict: dict mapping ion to ('gas', Emission_field_name)
        projection: 'edge-on' or 'face-on'
        cmap_range: (vmin, vmax) in km/s
        prefix: output path
        save_suffix: optional filename suffix
    """
    save_path = os.path.join(prefix, 'velocity_plots/delta_v_maps/')
    os.makedirs(save_path, exist_ok=True)
    f, grp, data_sources, width_value, res, min_res, bin_size_cm, unit_label = process_emission_maps(
        args, ds, refine_box, halo_dict, filter_type, filter_value,
        disk_file, shell_count, shell_path, unit_system, prefix, save_suffix, ions)
    ...
    cmap = cmr.get_sub_cmap('cmr.redshift', 0.1, 0.9)
    cmap.set_bad(cmap(0))

    # Add velocity fields in cm/s
    ds.add_field(('gas', 'vx_disk_cgs'),
        function=lambda field, data: data[('gas', 'vx_disk')].to('cm/s'),
        units='cm/s', sampling_type='cell', force_override=True)
    ds.add_field(('gas', 'vz_disk_cgs'),
        function=lambda field, data: data[('gas', 'vz_disk')].to('cm/s'),
        units='cm/s', sampling_type='cell', force_override=True)
    for projection in ['edge-on']:#, 'face-on']:
        #for ion1, ion2 in combinations(ions, 2):
        for ion2 in ions[1:]:
            ion1 = ions[0]
            try:
                emission_field1 = emission_field_dict[ion1]
                emission_field2 = emission_field_dict[ion2]
            except KeyError:
                print(f"Skipping {ion1}-{ion2}: missing emission field")
                continue

            if projection == 'edge-on':
                axis = ds.x_unit_disk
                velocity_field = ('gas', 'vx_disk_cgs')
                north_vector = ds.z_unit_disk
                xlabel, ylabel = 'Y (kpc)', 'Z (kpc)'
            elif projection == 'face-on':
                axis = ds.z_unit_disk
                velocity_field = ('gas', 'vz_disk_cgs')
                north_vector = ds.x_unit_disk
                xlabel, ylabel = 'X (kpc)', 'Y (kpc)'
            else:
                raise ValueError("Projection must be 'edge-on' or 'face-on'.")

            # Generate emissivity-weighted projections for both ions
            print('width_value:',width_value)
            proj1 = yt.ProjectionPlot(ds, axis, velocity_field,
                                    center=ds.halo_center_kpc, data_source=data_source,
                                    width=(int(width_value), 'kpc'), buff_size=[res, res],
                                    method='integrate', weight_field=emission_field1,
                                    north_vector=north_vector)
            proj2 = yt.ProjectionPlot(ds, axis, velocity_field,
                                    center=ds.halo_center_kpc, data_source=data_source,
                                    width=(int(width_value), 'kpc'), buff_size=[res, res],
                                    method='integrate', weight_field=emission_field2,
                                    north_vector=north_vector)

            vmap1 = proj1.frb[velocity_field].to('km/s').v
            vmap2 = proj2.frb[velocity_field].to('km/s').v
            delta_v = np.abs(vmap1 - vmap2)

            ######find coordinates
            # Find pixel indices where delta_v == 50 (use np.isclose to allow for float tolerance)
            # test_value = 40.0
            # matching_indices = np.argwhere(np.isclose(delta_v, test_value, atol=0.001))

            # # Compute pixel size in kpc
            # x_min, x_max, y_min, y_max = extent
            # pixel_size = (x_max - x_min) / delta_v.shape[1]  # assuming square pixels and same shape for x/y

            # # Convert pixel indices to coordinates in kpc
            # coords_kpc = []
            # for i, j in matching_indices:
            #     x_kpc = x_min + j * pixel_size + pixel_size / 2
            #     y_kpc = y_min + i * pixel_size + pixel_size / 2
            #     coords_kpc.append((x_kpc, y_kpc))

            # if coords_kpc:
            #     print(f"Coordinates where Δv = {test_value} km/s for {ion1} vs {ion2}:")
            #     for coord in coords_kpc:
            #         print(f"    x = {coord[0]:.2f} kpc, y = {coord[1]:.2f} kpc")
            # else:
            #     print(f"No pixels with Δv ≈ {test_value} km/s for {ion1} vs {ion2}")
            ...
            # Plot
            fig, ax = plt.subplots(figsize=(8, 6))
            im = ax.imshow(delta_v, origin='lower', extent=extent,
                        vmin=cmap_range[0], vmax=cmap_range[1], cmap='bwr')
            cbar = plt.colorbar(im, ax=ax)
            cbar.set_label(rf'$|v_{{los}}({ion1}) - v_{{los}}({ion2})|$ [km/s]')

            ax.set_xlabel(xlabel)
            ax.set_ylabel(ylabel)
            ax.set_title(rf'$\Delta v_{{los}}$: {ion1} vs {ion2} ({projection})')
            for (y_kpc, z_kpc) in target_positions:
                color = f"C{i % 10}"  # Use matplotlib's built-in cycle
                ax.plot(y_kpc, z_kpc, marker='s', markersize=8, markerfacecolor='none',
                        markeredgecolor=color, markeredgewidth=2)
            ...
            plt.tight_layout()
            fname = f"{snap}_delta_v_{ion1}_vs_{ion2}_{projection}_{region}{save_suffix}.png"
            plt.savefig(os.path.join(save_path, fname), dpi=300)
            plt.close()

        print(f"Saved delta-v maps for {projection} in region '{region}'.")
...
def plot_velocity_profiles_grid(hist_data_for_binwidth, ions,
    ...
    """
    Plot velocity-emissivity histograms for each ion in a single figure with subplots for all LOSs.
    """
    import math
    ...
    for ion in ions:
        ...
        for i, ((y_kpc, z_kpc), (bin_centers, hist)) in enumerate(hist_data_for_binwidth[ion].items()):
            if normalize:
            ...
            ax = axes[i]
            ax.plot(bin_centers, hist, color='black', lw=2)
            ax.set_title(f'y={y_kpc:.1f}, z={z_kpc:.1f}', fontsize=12)
            ax.set_xlim(-600, 400)
            if vmax:
                ax.set_ylim(0, vmax)
        ...
        fig.suptitle(f'{ion} LOS Velocity Profiles (bin = {bin_width_kms} km/s)', fontsize=16)
        fig.text(0.5, 0.04, 'LOS Velocity (km/s)', ha='center', fontsize=14)
        fig.text(0.04, 0.5, 'Normalized Emissivity' if normalize else 'Emissivity',
...
def load_and_calculate(snap, ions,scale_factor=None, unit_system='photons', filter_type=None, filter_value=None, res_arcsec=None):
    ...
    velocity_bin_widths = [200, 100, 30]
    # target_positions,data_source,region,velocity_hist_data_all, raw_data_all = emission_velocity(ds, refine_box, ions, ions_dict, prefix,
    #                   filter_value=filter_value, filter_type=filter_type, shell_count=shell_count,
    #                   bin_width_kms_list=velocity_bin_widths, apply_smoothing=False, sigma=0)
    target_positions,data_source,region,velocity_hist_data_all, raw_data_all = emission_velocity_with_pixel_size(ds, refine_box, ions, ions_dict, prefix,
    ...
    # plot_emissivity_weighted_velocity_maps_with_sightlines(ds, refine_box, snap, ions, target_positions,
    #                                                         unit_system=unit_system, filter_type=filter_type,
    #                                                         filter_value=filter_value, res_arcsec=res_arcsec)
    #plot_emission_maps_with_sightlines(ds, refine_box, snap, ions, target_positions, prefix,filter_value=filter_value, save_suffix='')
    # emission_field_dict = {ion: ('gas', f'Emission_{ions_dict[ion]}') for ion in ions}

    # make_delta_v_maps_between_ions(ds, refine_box, snap, ions, region, data_source,target_positions,
    #                                  emission_field_dict,prefix,filter_value=None, cmap_range=(0, 60),
    #                                  save_suffix='')
    # for bin_width in velocity_bin_widths:
    #     hist_data_for_binwidth = velocity_hist_data_all[bin_width]
    #     plot_velocity_comparison_all_pairs(hist_data_for_binwidth, ions,bin_width_kms=bin_width, normalize=True,
    #                                    smoothing=False, sigma=0)

    
    plot_3x3_velocity_comparison_grid(velocity_hist_data_all, ions,target_positions,
                                        bin_widths=[200, 100, 30],
                                        normalize=True, smoothing=False, sigma=0)

    for bin_width in velocity_bin_widths:
        plot_velocity_profiles_grid(velocity_hist_data_all[bin_width], ions,
