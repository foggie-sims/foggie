...
def map_ifu_to_grism(args,instrument):
   ...
   # xaspect_ratio = float(grism_nx) / float(ifu_nx)
    ...
    if args.snr>0:
        ...
        ifu_cube = ifu_cube + calculate_noise_flux(ifu_cube+sky_ifu,ifu_wavelength,instrument,args,False)  #only get noise
...
def calculate_noise_flux(ifu,wavelengths,instrument,args,return_err=True):
    ...
    if return_err: return np.divide(random_noise,flux_density_to_counts),np.divide(absolute_noise,flux_density_to_counts)
    
    return np.divide(random_noise,flux_density_to_counts)
...
instrument_background = instrument_background*np.ones(np.shape(grism))
instrument_background,instrument_err = calculate_noise_from_counts(instrument_background,args)
...
if args.snr>0:
    ...
    WriteFitsFile(grism,grism*0,args,"_mock")
