...
def parse_args():
    ...
    parser.add_argument('--set_res_auto', metavar='set_res_auto', type=bool, action='store', \
                        help='Set the pixel resolution automatically such that the maximum spatial resolution is resolved in the image plane by a factor of 5. Default is False.')
    parser.set_defaults(set_res_auto=False)  
    ...
    parser.set_defaults(make_simple_moment_maps=False) 
    ...
    parser.set_defaults(skip_full_datacube=False) 
    ...
    parser.set_defaults(skip_mock_datacube=False) 
    ...
    parser.add_argument('--clean_gain', metavar='clean_gain', type=float, action='store', \
                        help='What is the loop gain for the clean algorithm? Recommended 0.1-0.5. Default is 0.25')
    parser.set_defaults(clean_gain=0.25) 

    parser.add_argument('--max_iterations', metavar='max_iterations', type=float, action='store', \
    ...
    parser.add_argument('--log_min_column_density', metavar='log_min_column_density', type=float, action='store', \
                        help='What is the log of the minimum column density the noise model should be sensitive to? Default is 0.')
    ...
    parser.add_argument('--skip_spatial_filtering', metavar='skip_spatial_filtering', type=bool, action='store', \
                        help='Do you want to skip explicit spatial filtering and only estimate the power? Default is False')
    ...
    parser.add_argument('--object_name', metavar='object_name', type=str, action='store', \
                        help="What to name the object in the FITS file (9 chars). Default is 'foggie   '")
    parser.set_defaults(object_name='foggie   ') 
    ...
    parser.add_argument('--sigma_noise_level', metavar='sigma_noise_level', type=float, action='store', \
                        help="What sigma level to count as detection? Default is 5")
    ...
    parser.add_argument('--test_calibrator_clean', metavar='test_calibrator_clean', type=bool, action='store', \
                        help="Do you want to test the calibrator clean? Default is False.")
    ...
    parser.set_defaults(freq_range_mult=1) 
    ...
    parser.add_argument('--clean_wideband', metavar='clean_wideband', type=bool, action='store', \
                        help="Do you want clean all bands at once? Default is False.")
    parser.set_defaults(clean_wideband=False) 
    ...
    parser.add_argument('--primary_beam_FWHM_deg', metavar='primary_beam_FWHM_deg', type=float, action='store', \
                        help="What is the FWHM of the primary beam in degrees? Default is None.")
    ...
    parser.add_argument('--make_cell_mass_projection', metavar='make_cell_mass_projection', type=bool, action='store', \
                        help="Do you want to make a projection of the HI weighted cell mass at each pixel? Default is False")
    parser.set_defaults(make_cell_mass_projection=False) 
    ...
    parser.add_argument('--make_clump_cut_mask', metavar='make_clump_cut_mask', type=bool, action='store', \
                        help="Do you want to make a mask for the clump projection? Default is False")
    parser.set_defaults(make_clump_cut_mask=False)

    parser.add_argument("--clump_cut_file", metavar='clump_cut_file', type=str, action='store', \
                        help="What is the clump cut file to use?")
    parser.set_defaults(clump_cut_file=None)

    parser.add_argument('--clump_cut_suffix', metavar='clump_cut_suffix', type=str, action='store', \
                        help="What is the suffix to append to the clump cutoff file? Default is ''")
    parser.set_defaults(clump_cut_suffix='')

    parser.add_argument('--n_clumps_to_cut', metavar='n_clumps_to_cut', type=int, action='store', \
                        help="How many clumps do you want to cut? Default is 1")
    parser.set_defaults(n_clumps_to_cut=1)

    parser.add_argument('--clean_mask_filedir', metavar='clean_mask_filedir', type=str, action='store', \
                        help="What is your clean mask?")
    parser.set_defaults(clean_mask_filedir=None)

    parser.add_argument('--do_noiseless_clean', metavar='do_noiseless_clean', type=bool, action='store', \
                        help="Do cleaning on the noiseless datacube and then add appropriate noise. Default is False.")
    parser.set_defaults(do_noiseless_clean=False)
