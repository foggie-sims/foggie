...
def parse_args():
    ...
    parser.set_defaults(clump_dir='./clump_catalog/')
...
for leaf_id in leaf_clump_ids:
    ...
    leaf_masses.append(np.sum(gas_masses[np.isin(cell_ids, leaf_cell_ids)]).in_units('Msun').v)
...
plt.figure()
plt.plot(np.log10(dM), np.divide(N , dM), color='blue', alpha=0.7, label='Clump Mass Function', linewidth=2)
plt.xlabel('log Clump Mass (M$_{\\odot}$)',fontsize=labelsize)
plt.ylabel('Clump Mass Function [M$_{\\odot}^{-1}$]',fontsize=labelsize)
#plt.xscale('log')
plt.yscale('log')
plt.savefig(args.output_dir + GalName+"_"+args.snapshot+"_"+args.run+"_clump_mass_function.png",dpi=300,bbox_inches='tight')


plt.figure()
plt.hist(np.log10(np.array(leaf_masses)), bins=nbins,range=[min_mass,max_mass], color='blue', alpha=0.7, edgecolor='black',density=True)
plt.xlabel('log Clump Mass (M$_{\\odot}$)',fontsize=labelsize)
plt.ylabel('Clump Mass Function [M$_{\\odot}^{-1}$]',fontsize=labelsize)
#plt.xscale('log')
plt.yscale('log')
...
plt.savefig(args.output_dir + GalName+"_"+args.snapshot+"_"+args.run+"_clump_mass_histogram.png",dpi=300,bbox_inches='tight')
