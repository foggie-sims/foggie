...
def plot_cloud_size_and_masses():
    ...
    nref11f_h1_cloud_cells = []

    nref11f_o6temps = []
    nref11f_o6vc = []
    nref11f_o6dv = []
    nref11f_o6cloudcolumns = []
    nref11f_o6_cloud_cells = []

    nref11f_si2temps = []
    nref11f_si2vc = []
    nref11f_si2dv = []
    nref11f_si2cloudcolumns = []
    nref11f_si2_cloud_cells = []

    nref11f_c4temps = []
    nref11f_c4vc = []
    nref11f_c4dv = []
    nref11f_c4cloudcolumns = []
    nref11f_c4_cloud_cells = []

    nref11f_si4temps = []
    nref11f_si4vc = []
    nref11f_si4dv = []
    nref11f_si4cloudcolumns = []
    nref11f_si4_cloud_cells = []

    for file in filelist:
        nref11f_size_dict = pickle.load( open( file, "rb" ) )
        raydf = nref11f_size_dict['ray_df']
        if "rd0020" in file:
            z = 2.0
        if "rd0018" in file:
            z = 2.5
        ...
        if (nref11f_size_dict['nh1'] > h1_limit):
            if np.max(nref11f_size_dict['h1_kpcsizes']) < 200:
                ## loop through the clouds
                for i, item in enumerate(nref11f_size_dict['h1_coldens']):
                    this_cloud = raydf[raydf['h1_cloud_flag'] == (i+1)]
                    m = m_p
                    dv = (np.max(this_cloud[this_velocity]) -
                          np.min(this_cloud[this_velocity])) * u.km / u.s
                    T = sum(this_cloud['temperature'] * this_cloud['H_p0_number_density']*this_cloud['dx'] / sum(this_cloud['H_p0_number_density']*this_cloud['dx'])) * u.K
                    vc = np.sqrt(k_B*T / m).to('km/s')
                    nref11f_h1temps.append(T.value)
                    nref11f_h1dv.append(dv.value)
                    nref11f_h1vc.append(vc.value)
                    nref11f_h1cloudcolumns.append(item)
                    nref11f_h1_cloud_cells.append(len(this_cloud))

        if (nref11f_size_dict['nsi2'] > si2_limit):
            if np.max(nref11f_size_dict['si2_kpcsizes']) < 200:
                ## loop through the clouds
                for i, item in enumerate(nref11f_size_dict['si2_coldens']):
                    this_cloud = raydf[raydf['si2_cloud_flag'] == (i+1)]
                    m = simass * m_p
                    dv = (np.max(this_cloud[this_velocity]) -
                          np.min(this_cloud[this_velocity])) * u.km / u.s
                    T = sum(this_cloud['temperature'] * this_cloud['Si_p1_number_density']*this_cloud['dx'] / sum(this_cloud['Si_p1_number_density']*this_cloud['dx'])) * u.K
                    vc = np.sqrt(k_B*T / m).to('km/s')
                    nref11f_si2temps.append(T.value)
                    nref11f_si2dv.append(dv.value)
                    nref11f_si2vc.append(vc.value)
                    nref11f_si2cloudcolumns.append(item)
                    nref11f_si2_cloud_cells.append(len(this_cloud))

        if (nref11f_size_dict['no6'] > o6_limit):
            if np.max(nref11f_size_dict['o6_kpcsizes']) < 200:
                ## loop through the clouds
                for i, item in enumerate(nref11f_size_dict['o6_coldens']):
                    this_cloud = raydf[raydf['o6_cloud_flag'] == (i+1)]
                    m = omass * m_p
                    dv = (np.max(this_cloud[this_velocity]) -
                          np.min(this_cloud[this_velocity])) * u.km / u.s
                    T = sum(this_cloud['temperature'] * this_cloud['O_p5_number_density']*this_cloud['dx'] / sum(this_cloud['O_p5_number_density']*this_cloud['dx'])) * u.K
                    vc = np.sqrt(k_B*T / m).to('km/s')
                    nref11f_o6temps.append(T.value)
                    nref11f_o6dv.append(dv.value)
                    nref11f_o6vc.append(vc.value)
                    nref11f_o6cloudcolumns.append(item)
                    nref11f_o6_cloud_cells.append(len(this_cloud))

        if (nref11f_size_dict['nc4'] > c4_limit):
            if np.max(nref11f_size_dict['c4_kpcsizes']) < 200:
                ## loop through the clouds
                for i, item in enumerate(nref11f_size_dict['c4_coldens']):
                    this_cloud = raydf[raydf['c4_cloud_flag'] == (i+1)]
                    m = cmass * m_p
                    dv = (np.max(this_cloud[this_velocity]) -
                          np.min(this_cloud[this_velocity])) * u.km / u.s
                    T = sum(this_cloud['temperature'] * this_cloud['C_p3_number_density']*this_cloud['dx'] / sum(this_cloud['C_p3_number_density']*this_cloud['dx'])) * u.K
                    vc = np.sqrt(k_B*T / m).to('km/s')
                    nref11f_c4temps.append(T.value)
                    nref11f_c4dv.append(dv.value)
                    nref11f_c4vc.append(vc.value)
                    nref11f_c4cloudcolumns.append(item)
                    nref11f_c4_cloud_cells.append(len(this_cloud))

    nref11f_h1_sampling = np.array(nref11f_h1dv) / (np.array(nref11f_h1vc))
    nref11f_si2_sampling = np.array(nref11f_si2dv) / (np.array(nref11f_si2vc))
    nref11f_c4_sampling =  np.array(nref11f_c4dv) / (np.array(nref11f_c4vc))
    nref11f_o6_sampling = np.array(nref11f_o6dv) / (np.array(nref11f_o6vc))
    nref11f_h1_ratio = np.array(nref11f_h1_sampling)  / ((np.array(nref11f_h1_cloud_cells) - 1)*sigma_ratio)
    nref11f_si2_ratio = np.array(nref11f_si2_sampling)  / ((np.array(nref11f_si2_cloud_cells) - 1)*sigma_ratio)
    nref11f_c4_ratio = np.array(nref11f_c4_sampling)  / ((np.array(nref11f_c4_cloud_cells) - 1)*sigma_ratio)
    nref11f_o6_ratio = np.array(nref11f_o6_sampling)  / ((np.array(nref11f_o6_cloud_cells) - 1)*sigma_ratio)
    nref11f_h1_ratio[nref11f_h1_ratio == np.nan] = 1e6
    nref11f_si2_ratio[nref11f_si2_ratio == np.nan] = 1e6
    nref11f_c4_ratio[nref11f_c4_ratio == np.nan] = 1e6
    nref11f_o6_ratio[nref11f_o6_ratio == np.nan] = 1e6
    nref11f_h1_ratio[nref11f_h1_ratio == np.inf] = 1e6
    nref11f_si2_ratio[nref11f_si2_ratio == np.inf] = 1e6
    nref11f_c4_ratio[nref11f_c4_ratio == np.inf] = 1e6
    nref11f_o6_ratio[nref11f_o6_ratio == np.inf] = 1e6
    ...
    axtop.grid(True)
    fig.text(0.5, 0.92, 'standard resolution', fontsize=32, ha='left')


    axmid.hist(nref10f_h1_ratio, hist_bins, range=(0, 20), normed=True, cumulative=-1,edgecolor=hi_color, lw=3,histtype='step',align='mid', label='H I')
    axmid.hist(nref10f_si2_ratio, hist_bins, range=(0, 20), normed=True, cumulative=-1, edgecolor=si2_color,  lw=3, histtype='step', align='mid',label='Si II')
    axmid.hist(nref10f_c4_ratio, hist_bins, range=(0, 20), normed=True, cumulative=-1, edgecolor=c4_color, lw=3,histtype='step',align='mid',label='C IV')
    axmid.hist(nref10f_o6_ratio, hist_bins, range=(0, 20), normed=True, cumulative=-1,edgecolor=ovi_color, lw=3,histtype='step', align='mid',label='O VI')
    axmid.set_xticklabels(())
    axmid.set_yticks((0.2, 0.4, 0.6,0.8,1))
    axmid.set_yticklabels(('0.2','0.4','0.6','0.8','1'))
    axmid.grid(True)
    fig.text(0.5, 0.62, '157pc resolution', fontsize=32, ha='left')


    axbot.hist(nref11f_h1_ratio, hist_bins, range=(0, 20), normed=True, cumulative=-1,edgecolor=hi_color, lw=3,histtype='step',align='mid', label='H I')
    axbot.hist(nref11f_si2_ratio, hist_bins, range=(0, 20), normed=True, cumulative=-1, edgecolor=si2_color,  lw=3, histtype='step', align='mid',label='Si II')
    axbot.hist(nref11f_c4_ratio, hist_bins, range=(0, 20), normed=True, cumulative=-1, edgecolor=c4_color, lw=3,histtype='step',align='mid',label='C IV')
    axbot.hist(nref11f_o6_ratio, hist_bins, range=(0, 20), normed=True, cumulative=-1,edgecolor=ovi_color, lw=3,histtype='step', align='mid',label='O VI')
    ...
    axbot.grid(True)
    fig.text(0.5, 0.32, '78pc resolution', fontsize=32, ha='left')
    ...
    # plt.tight_layout()
    plt.savefig(output_dir + 'cloud_velocities_z25.png')
    plt.savefig(output_dir + 'cloud_velocities_z25.pdf')
