...
def make_hi_plots(ds, prefix, **kwargs):
    ...
    width = kwargs.get("width", default_width)
    resolution = kwargs.get("resolution", (1048,1048)) # correct for the nref11f box
    basename = prefix + 'ions/' + ds.basename + '_HI' + appendix
...
def make_c4_plots(ds, prefix, **kwargs):
    ...
    width = kwargs.get("width", default_width)
    resolution = kwargs.get("resolution", (1048,1048)) # correct for the nref11f box
    basename = prefix + 'ions/' + ds.basename + '_CIV' + appendix
    if not (os.path.exists(prefix + 'ions/' )):
        os.system("mkdir " + prefix + 'ions/' )
    for ax in axis:
        if not (os.path.exists(prefix + 'ions/' )):
    ...
    for snap in outs:
        ...
        if not args.noslices:
            if args.all or args.physical or args.density or args.slices:
                para_funcs.append(lambda: make_density_slice_plot(ds, prefix, axis=axis, center=center, box=refine_box, \
                                   width=(refine_width-10.), appendix="_refine"))

            if args.all or args.physical or args.metals or args.slices:
                para_funcs.append(lambda: make_metal_slice_plot(ds, prefix, axis=axis, center=center, box=refine_box, \
                                      width=(refine_width-10.), appendix="_refine"))

            if args.all or args.physical or args.slices:
                para_funcs.append(lambda: make_temperature_slice_plot(ds, prefix, axis=axis, center=center, box=refine_box, \
                                      width=(refine_width-10.), appendix="_refine"))
                para_funcs.append(lambda: make_entropy_slice_plot(ds, prefix, axis=axis, center=center, box=box, width=width, appendix="_box"))
                para_funcs.append(lambda: make_entropy_slice_plot(ds, prefix, axis=axis, center=refine_box_center, \
        ...
        if args.all or args.physical or args.density:
            ...
            para_funcs.append(lambda: make_density_projection_plot(ds, prefix, axis=axis, center=center, box=box, \
                               width=width, appendix="_box"))
            para_funcs.append(lambda: make_density_projection_plot(ds, prefix, axis=axis, center=refine_box_center, box=refine_box, \
                               width=refine_width, appendix="_refine"))

        if args.all or args.physical:
            para_funcs.append(lambda: make_temperature_projection_plot(ds, prefix, axis=axis, center=refine_box_center,\
                             box=refine_box, width=refine_width, appendix="_refine"))
            para_funcs.append(lambda: make_temperature_projection_plot(ds, prefix, axis=axis, center=center, box=box, width=width, appendix="_box"))

        if args.all or args.physical or args.metals:
            para_funcs.append(lambda: make_metal_projection_plot(ds, prefix, axis=axis, center=refine_box_center,\
                             box=refine_box, width=refine_width, appendix="_refine"))
            para_funcs.append(lambda: make_metal_projection_plot(ds, prefix, axis=axis, center=center, box=box, width=width, appendix="_box"))

        if args.all or args.ions or args.hi:
            para_funcs.append(lambda: make_hi_plots(ds, prefix,  center=refine_box_center, \
                          box=refine_box, width=refine_width, appendix="_refine"))
            para_funcs.append(lambda: make_hi_plots(ds, prefix, axis=axis, center=center, box=box, width=width, appendix="_box"))

        if args.all or args.ions or args.ovi:
            para_funcs.append(lambda: make_o6_plots(ds, prefix, axis=axis, center=refine_box_center, \
                          box=refine_box, width=refine_width, appendix="_refine"))
            para_funcs.append(lambda: make_o6_plots(ds, prefix, axis=axis, center=center, box=box, width=width, appendix="_box"))

        if args.all or args.ions or args.civ:
            para_funcs.append(lambda: make_c4_plots(ds, prefix, axis=axis, center=refine_box_center, \
                          box=refine_box, width=refine_width, appendix="_refine"))
            para_funcs.append(lambda: make_c4_plots(ds, prefix, axis=axis, center=center, box=box, width=width, appendix="_box"))

        if args.all or args.ions or args.silicon:
            para_funcs.append(lambda: make_si2_plots(ds, prefix, axis=axis, center=refine_box_center, box=refine_box, width=refine_width, appendix="_refine"))
            para_funcs.append(lambda: make_si2_plots(ds, prefix, axis=axis, center=center, box=box, width=width, appendix="_box"))
            para_funcs.append(lambda: make_si3_plots(ds, prefix, axis=axis, center=refine_box_center, box=refine_box, width=refine_width, appendix="_refine"))
            para_funcs.append(lambda: make_si3_plots(ds, prefix, axis=axis, center=center, box=box, width=width, appendix="_box"))

    from multiprocessing import Pool

    pool = Pool(processes=4)

    # Multi-process can't handle closures as the function to which the
    # arguments are passed. Must create a named function.
    def worker(func):
        return func()

    res = pool.apply_async(worker, para_funcs)
    res.get()
