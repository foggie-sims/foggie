...
def plot_script(halo, run, axis, firstsnap, lastsnap, trackname):
    ...
    for n in outs:
        ...
        refine_box_center = [0.5*(x_left+x_right),
                             0.5*(y_left+y_right),
                             0.5*(z_left+z_right)]
        refine_box = ds.r[x_left:x_right, y_left:y_right, z_left:z_right]

        box = ds.r[center[0]-250./143886:center[0]+250./143886, center[1]-250. /
                   143886.:center[1]+250./143886., center[2]-250./143886.:center[2]+250./143886.]

        p = yt.ProjectionPlot(ds, axis, 'density', center=center,
                              data_source=box, width=(wide, 'kpc'))
        p.annotate_timestamp(corner='upper_left', redshift=True,
                                draw_inset_box=True)
        p.set_cmap(field="density", cmap='RdBu')
        p.set_zlim("density", 1e-6, 0.1)
        p.save(prefix+'plots/density_projection_map/'+axis+'/'+strset+'_box')

        p = yt.ProjectionPlot(ds, axis, 'density', center=[
                              refine_box_center[0], refine_box_center[1],
                              refine_box_center[2]], data_source=refine_box,
                              width=(wide, 'kpc'))
        ...
        p = yt.SlicePlot(ds, axis, 'metallicity', center=[
                         refine_box_center[0], center[1], center[2]], data_source=box, width=(wide, 'kpc'))
        p.annotate_timestamp(corner='upper_left', redshift=True, draw_inset_box=True)
        ...
        p = yt.ProjectionPlot(ds, axis, 'H_p0_number_density', center=center,
                              data_source=box, width=(wide, 'kpc'))
        p.annotate_timestamp(corner='upper_left', redshift=True, draw_inset_box=True)
        ...
        p = yt.ProjectionPlot(ds, axis, 'H_p0_number_density', center=center,
                              data_source=refine_box, width=(wide, 'kpc'))
        p.annotate_timestamp(corner='upper_left', redshift=True, draw_inset_box=True)
        p.set_zlim("H_p0_number_density", 1e11, 1e17)
        p.save(prefix+'plots/HI_refine_map/'+axis+'/'+strset+'_HI_refine')

        p = yt.ProjectionPlot(ds, axis, "O_p5_number_density", center=center,
                              data_source=box, width=(wide, 'kpc'))
        p.annotate_timestamp(corner='upper_left', redshift=True, draw_inset_box=True)
        ...
        p = yt.ProjectionPlot(ds, axis, "C_p3_number_density", center=center,
                              data_source=box, width=(wide, 'kpc'))
        p.annotate_timestamp(corner='upper_left', redshift=True, draw_inset_box=True)
        p.set_zlim("C_p3_number_density", 1e11, 1e15)
        p.save(prefix+'plots/CIV_projection_map/'+axis+'/'+strset+'_CIV')

        p = yt.ProjectionPlot(ds, axis, "Si_p2_number_density", center=center,
                              data_source=box, width=(wide, 'kpc'))
        p.annotate_timestamp(corner='upper_left', redshift=True, draw_inset_box=True)
        p.set_zlim("Si_p2_number_density", 1e11, 1e15)
        p.save(prefix+'plots/SiIII_projection_map/'+axis+'/'+strset+'_SiIII')

        p = yt.ProjectionPlot(ds, axis, 'temperature', center=[
                              refine_box_center[0], refine_box_center[1], refine_box_center[2]], data_source=refine_box, width=(wide, 'kpc'))
        p.annotate_timestamp(corner='upper_left', redshift=True, draw_inset_box=True)
        ...
        p = yt.SlicePlot(ds, axis, 'entropy', center=[
                         refine_box_center[0], refine_box_center[1], refine_box_center[2]], data_source=refine_box, width=(wide, 'kpc'))
        p.annotate_timestamp(corner='upper_left', redshift=True, draw_inset_box=True)
        ...
        p = yt.SlicePlot(ds, axis, 'temperature', center=[
                         refine_box_center[0], refine_box_center[1], refine_box_center[2]], data_source=refine_box, width=(wide, 'kpc'))
        p.annotate_timestamp(corner='upper_left', redshift=True, draw_inset_box=True)
        p.set_cmap(field="temperature", cmap='rainbow')
