...
The initial conditions first need to be generated by MUSIC. See
`Generating New Zoom Initial Conditions <generating_new_ICs.html>`_ for the
automated pipeline that does this, which also submits and chains the Enzo runs
for each refinement level. This produces one of each of the following files
